<template>
    <div class="popup" :class="{'playMode':playModeVisible}">
        <!-- 播放列表 -->
        <mt-popup position="bottom" v-model="playListVisible" class="playListSheet">
            <div class="playList-header">
                <div class="playList-mode-btn left" @click="clearPlayList">清空</div>
            ...
            <ul class="playList" v-if="playList&&playList.length>0">
                <li class="playList-item" v-for="(item, index) in playList" :key="index" :class="{'playing':audio.data.sound.id===item.sound.id}" @click="changeMusic(item)">
                    <div class="item-name">
                        <!-- icon -->
            ...
            <div class="playList-nothing" v-else>还没有播放过节目呦~</div>
        </mt-popup>

        <!-- 播放模式 -->
        <mt-popup position="bottom" v-model="playModeVisible" class="playModeSheet">
            <div class="playModeList">
                <mt-cell class="playMode-item" v-for="(item, index) in playModeOptions" :key="index" :class="{'active':playMode===item.value}" :title="item.label" @click.native="changeMode(item.value)">
...
export default {
    ...
    methods: {
        ...
        // 切换播放模式
        changeMode(val) {
            console.log('changeMode--val='+val);
